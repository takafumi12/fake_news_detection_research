...
def get_nikkei_news_data():
    ...
    data_path_pkl = os.path.join('../data', 'scraping_data', f'nikkei_news_title_list.pkl')
    df_nikkei_news_title_list = [re.sub(r'（.*）', '', s) for s in df_nikkei_news_title['title'].tolist()]
    print(df_nikkei_news_title_list)
    Util.dump(df_nikkei_news_title_list , data_path_pkl)
    data_path_csv = os.path.join('../data', 'scraping_data', f'nikkei_news_data.csv')
